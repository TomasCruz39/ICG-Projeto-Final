...
function addBoxCollider(x, z, sx, sz) {
...
function addWall(x, z, sx, sz, height = 0.9) {
...
function addWindmill(x, z, armLength = 2.1, armThickness = 0.24, speed = 2.5) {
	...
	addObject(group);
	movingObstacles.push({ type: "windmill", mesh: group, speed, armLength, armThickness });
}
...
function buildMapMedium() {
	...
	ramp.position.set(-2.0, 0.14, 2.4); ramp.rotation.x = Math.PI / 14;
...
function resolveObstacleCollision(obstacle) {
	if (obstacle.type === "slideBox") {
		const box = { minX: obstacle.mesh.position.x - obstacle.halfX, maxX: obstacle.mesh.position.x + obstacle.halfX, minZ: obstacle.mesh.position.z - obstacle.halfZ, maxZ: obstacle.mesh.position.z + obstacle.halfZ };
...
function updateBall(dt) {
	...
	if (game.currentMap === 1) {
		if (ball.position.x > -3.4 && ball.position.x < -0.6 && ball.position.z > 1.3 && ball.position.z < 3.5) {
			ball.position.y = ball.radius + ((3.5 - ball.position.z) / 2.2 * 0.42);
		} else { ball.position.y = ball.radius; }
...
window.addEventListener("keydown", (e) => {
	...
	else if (key === "f") game.freeCam = !game.freeCam;
});
